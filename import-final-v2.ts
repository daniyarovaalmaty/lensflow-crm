...
async function main() {
    console.log("Starting FINAL V2 import...");
    ...
    function findDbProduct(excelName: string) {
        ...
        // Ensure V78C-SR maps to V78С-SR
        if (excelName.includes('V78C-SR')) {
            let p = dbProducts.find(p => p.name.includes('V78С-SR'));
            if (p) return p;
        }
        ...
        // Try exact split match for VOLK items to avoid VAC catching V78C etc.
        for (let p of allVolk) {
            if (p.name.toUpperCase().startsWith(n.toUpperCase() + ' ')) return p;
        }
    ...
    console.log("FINAL IMPORT V2 COMPLETE!");
}
main();
