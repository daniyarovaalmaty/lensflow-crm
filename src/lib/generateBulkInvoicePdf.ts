...
export async function generateBulkInvoicePdf(orders: InvoiceOrder[]): Promise<void> {
    ...
    // Группируем заказы по компании (филиалу)
    const groupedOrders = new Map<string, InvoiceOrder[]>();
    for (const order of orders) {
        const clientName = order.contract?.client?.name || order.company || 'Покупатель не указан';
        if (!groupedOrders.has(clientName)) {
            groupedOrders.set(clientName, []);
        }
        groupedOrders.get(clientName)!.push(order);
    }

    let isFirstPage = true;

    for (const [clientName, branchOrders] of Array.from(groupedOrders.entries())) {
        if (!isFirstPage) {
            doc.addPage();
        }
        isFirstPage = false;

        // Берем данные первого заказа из группы для формирования шапки
        const firstOrder = branchOrders[0];
        
        const providerName = firstOrder.contract?.provider?.name || firstOrder.distributor_org?.name || firstOrder.lab_org?.name || 'ТОО "MedInnVision"';
        const providerInn = firstOrder.contract?.provider?.inn || firstOrder.distributor_org?.inn || firstOrder.lab_org?.inn || '970121400808';
        const providerAddress = firstOrder.contract?.provider?.address || firstOrder.distributor_org?.address || firstOrder.lab_org?.address || 'Алматинская обл., Талгарский р-он, с. Талгар, ул. БЕРЕГОВАЯ, д. 72';
        const providerBank = firstOrder.contract?.provider?.bankName || firstOrder.distributor_org?.bankName || firstOrder.lab_org?.bankName || 'АО "Народный Банк Казахстана"';
        const providerBik = firstOrder.contract?.provider?.bik || firstOrder.distributor_org?.bik || firstOrder.lab_org?.bik || 'HSBKKZKX';
        const providerIban = firstOrder.contract?.provider?.iban || firstOrder.distributor_org?.iban || firstOrder.lab_org?.iban || 'KZ48601A861003807741';

        const clientInn = firstOrder.contract?.client?.inn || firstOrder.optic_inn || (firstOrder as any).inn || '';
        const clientAddress = firstOrder.contract?.client?.address || firstOrder.optic_address || (firstOrder as any).delivery_address || '';

        let contractStr = '________________________________________';
        if (firstOrder.contract) {
            const cDate = new Date(firstOrder.contract.date);
            contractStr = `№${firstOrder.contract.number} от ${cDate.toLocaleDateString('ru-RU')} г.`;
        }

        const formatter = new Intl.DateTimeFormat('ru-RU', { day: '2-digit', month: 'long', year: 'numeric' });
        const dateStr = formatter.format(new Date()).replace(' г.', '');
        ...
        doc.setFontSize(14);
        doc.setFont('Roboto', 'bold');
        doc.text(`Счет на оплату от ${dateStr} г.`, pageWidth / 2, currentY, { align: 'center' });
        currentY += 3;
        ...
        for (const order of branchOrders) {
            ...
            if (odQty > 0) {
                ...
                tableRows.push([
                    rowNum++,
                    `${order.document_name_od || 'Линзы контактные'} (Заказ №${order.order_id})`,
                    odQty,
                    'шт',
                    fmt(odUnitPrice),
                    fmt(lineTotal)
            ...
            if (osQty > 0) {
                ...
                tableRows.push([
                    rowNum++,
                    `${order.document_name_os || 'Линзы контактные'} (Заказ №${order.order_id})`,
                    osQty,
                    'шт',
                    fmt(osUnitPrice),
                    fmt(lineTotal)
            ...
            for (const prod of additionalProducts) {
                ...
                tableRows.push([
                    rowNum++,
                    `${prod.name} (Заказ №${order.order_id})`,
                    pQty,
                    'шт',
                    fmt(pPrice),
                    fmt(lineTotal)
            ...
            grandTotal += order.total_price || (afterDiscount + urgentAmt);
        }
        ...
        if (totalDiscountAmt > 0) {
            ...
            finalTableData.push(['', '', '', '', 'Скидка:', `-${fmt(totalDiscountAmt)}`]);
        }
        if (totalUrgentAmt > 0) {
            finalTableData.push(['', '', '', '', 'Срочность (+25%):', `+${fmt(totalUrgentAmt)}`]);
        }
    ...
    doc.save(`Общий_счет_на_оплату.pdf`);
}
