...
import {
    ...
    UserPlus, Clock, Instagram, MessageSquare, Droplets, Repeat
} from 'lucide-react';
...
const STAGES = [
    { key: 'checkup', label: 'Ждут осмотра', icon: Calendar, color: 'bg-blue-100 text-blue-700' },
    { key: 'supplies', label: 'Нужны растворы', icon: Droplets, color: 'bg-indigo-100 text-indigo-700' },
    { key: 'renewal', label: 'Замена линз', icon: Repeat, color: 'bg-violet-100 text-violet-700' },
    { key: 'retention_dialog', label: 'В диалоге', icon: MessageCircle, color: 'bg-amber-100 text-amber-700' },
    { key: 'retention_success', label: 'Заказ оформлен', icon: TrendingUp, color: 'bg-green-100 text-green-700' },
    { key: 'retention_lost', label: 'Отток', icon: X, color: 'bg-gray-100 text-gray-700' },
];
...
export default function SalesPipelinePage() {
    ...
    const activeLeads = leads.filter(l => !['retention_success', 'retention_lost'].includes(l.stage)).length;
    const convertedCount = stageCounts['retention_success'] || 0;
    ...
    const handleAddLead = async () => {
        ...
        try {
            await fetch('/api/crm/leads', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ phone: newPhone, name: newName || null, city: newCity || null, source: 'manual', funnel: 'retention' }),
            });
                ...
                <div className="flex items-center justify-between mb-4">
                    <div>
                        <h1 className="text-lg sm:text-2xl font-bold text-gray-900 flex items-center gap-2">
                            <Repeat className="w-6 h-6 text-blue-600" /> Постоянные клиенты
                        </h1>
                        <p className="text-sm text-gray-500 mt-0.5">
                            {activeLeads} активных · {convertedCount} успешных
                        </p>
                    </div>
