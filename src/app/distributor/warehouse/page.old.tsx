...
type Tab = 'stock' | 'receive' | 'sale' | 'movements' | 'documents' | 'inventory';
...
export default function WarehousePage() {
    ...
    const [saleItems, setSaleItems] = useState<Array<{
        productId: string; quantity: string; serialMode: 'auto' | 'manual';
        manualSerials: string; price: string; color: string; size: string;
    }>>([]);
    ...
    const [scannerMode, setScannerMode] = useState<'search' | 'receive' | 'sale' | 'writeoff' | 'inventory'>('search');
    ...
    const handleScanResult = (code: string) => {
        ...
        if (scannerMode === 'search') {
            ...
            const existingIdx = saleItems.findIndex(ri => ri.productId === found.id);
            if (existingIdx > -1) {
                ...
                setSaleItems(prev => [
                    ...prev,
                    {
                        ...
                        serialMode: 'auto',
                        manualSerials: '',
                        price: String(found.retailPrice),
                        color: '',
                        size: '',
                    }
                ]);
            }
            alert(`✅ Товар добавлен в реализацию: ${found.name} (1 шт.)`);
    ...
    const loadData = async () => {
        ...
        try {
            ...
            if (tab === 'sale') {
                const res = await fetch('/api/distributor/counterparties?t=' + Date.now());
    ...
    const addSaleItem = () => {
        setSaleItems([...saleItems, {
            productId: products[0]?.id || '', quantity: '1', serialMode: 'auto',
            manualSerials: '', price: '', color: '', size: '',
        }]);
    };

    const updateSaleItem = (idx: number, field: string, value: any) => {
        const updated = [...saleItems];
        (updated[idx] as any)[field] = value;
        setSaleItems(updated);
    };
    ...
    const handleSaleSubmit = async () => {
        ...
        const customer = counterparties.find(c => c.id === saleCustomerId);
        setSaving(true);
        try {
            const items = saleItems.map(ri => ({
                productId: ri.productId,
                quantity: Number(ri.quantity) || 1,
                serialNumbers: ri.serialMode === 'manual' && ri.manualSerials
                    ? ri.manualSerials.split('\n').map(s => s.trim()).filter(Boolean)
                    : undefined,
                price: ri.price ? Number(ri.price) : undefined,
                color: ri.color || undefined,
                size: ri.size || undefined,
            }));
            ...
            if (res.ok) {
                const result = await res.json();
                alert(`🎉 Реализация оформлена: ${result.document?.documentNumber || ''}`);
                setSaleItems([]);
                setSaleCustomerId('');
                setSaleNotes('');
                setTab('stock');
                ...
                <div className="max-w-7xl mx-auto px-4 sm:px-6 py-6">
                    ...
                    <div className="flex gap-1 bg-gray-100 p-1 rounded-xl">
                        {([
                            ...
                            { key: 'sale', label: 'Реализация', icon: ArrowUpFromLine },
                ...
                {tab === 'sale' && (
                    <div className="max-w-2xl">
                        <div className="bg-white rounded-2xl border border-gray-100 p-6">
                            <div className="flex items-center justify-between mb-4">
                                <h2 className="text-lg font-bold text-gray-900 flex items-center gap-2">
                                    <ArrowUpFromLine className="w-5 h-5 text-blue-600" /> Реализация
                                </h2>
                                <button 
                                    onClick={() => { setScannerMode('sale'); setShowScanner(true); }}
                                    className="flex items-center gap-1.5 px-3 py-1.5 border border-gray-200 text-gray-600 hover:bg-gray-50 rounded-xl text-xs font-bold transition-all"
                                >
                                    <Camera className="w-3.5 h-3.5 text-primary-500" /> Сканировать линзу
                                </button>
                            ...
                            <div className="mb-4">
                                ...
                                <select 
                                    ...
                                    className="w-full border border-gray-200 rounded-xl px-3 py-2.5 text-sm focus:ring-2 focus:ring-primary-500 bg-white"
                                >
                                    <option value="">Выберите контрагента (необязательно)</option>
                                    {counterparties.map(c => (
                                        <option key={c.id} value={c.id}>{c.name} {c.bin ? `(БИН/ИИН: ${c.bin})` : ''}</option>
                                    ))}
                                </select>
                            </div>
                            ...
                            <div className="space-y-4 mb-4">
                                {saleItems.map((item, idx) => (
                                    <div key={idx} className="border border-gray-100 rounded-xl p-4 relative">
                                        <button onClick={() => removeSaleItem(idx)} className="absolute top-2 right-2 text-gray-400 hover:text-red-500">
                                            <X className="w-4 h-4" />
                                        </button>
                                        ...
                                        <div className="grid grid-cols-2 gap-3 mb-3">
                                            <div className="col-span-2">
                                                ...
                                                <ProductSearchSelect
                                                    products={products}
                                                    value={item.productId}
                                                    onChange={(pid) => updateSaleItem(idx, 'productId', pid)}
                                                />
                                            </div>
                                            <div>
                                                <label className="block text-xs font-medium text-gray-500 mb-1">Количество</label>
                                                <input type="number" min="1" value={item.quantity} onChange={e => updateSaleItem(idx, 'quantity', e.target.value)}
                                                    className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm" />
                                            </div>
                                            <div>
                                                <label className="block text-xs font-medium text-gray-500 mb-1">Цена продажи (₸)</label>
                                                <input type="number" value={item.price} onChange={e => updateSaleItem(idx, 'price', e.target.value)}
                                                    placeholder="По умолчанию розничная" className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm" />
                                            </div>
                                        </div>
                                        ...
                                        {products.find(p => p.id === item.productId)?.trackSerials && (
                                            <div className="mt-3">
                                                <label className="block text-xs font-medium text-gray-500 mb-1">Серийные номера (опционально)</label>
                                                <div className="flex gap-2 mb-2">
                                                    <button onClick={() => updateSaleItem(idx, 'serialMode', 'auto')}
                                                        className={`px-3 py-1.5 rounded-lg text-xs font-medium ${item.serialMode === 'auto' ? 'bg-primary-100 text-primary-700' : 'bg-gray-100 text-gray-500'}`}>
                                                        Авто (любые)
                                                    </button>
                                                    <button onClick={() => updateSaleItem(idx, 'serialMode', 'manual')}
                                                        className={`px-3 py-1.5 rounded-lg text-xs font-medium ${item.serialMode === 'manual' ? 'bg-primary-100 text-primary-700' : 'bg-gray-100 text-gray-500'}`}>
                                                        Ручной ввод
                                                    </button>
                                                </div>
                                                {item.serialMode === 'manual' && (
                                                    <textarea value={item.manualSerials} onChange={e => updateSaleItem(idx, 'manualSerials', e.target.value)}
                                                        placeholder="По одному на строку:&#10;SN-001&#10;SN-002" rows={3}
                                                        className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm resize-none" />
                                                )}
                                            </div>
                            ...
                            <button onClick={addSaleItem} className="w-full py-2.5 border-2 border-dashed border-gray-200 rounded-xl text-sm text-gray-500 hover:border-primary-300 hover:text-primary-600 transition-colors mb-4">
                                + Добавить товар
                            </button>
                            ...
                            <div className="mb-4">
                                ...
                                <textarea value={saleNotes} onChange={e => setSaleNotes(e.target.value)}
                                    placeholder="Комментарий к реализации..." rows={2}
                                    className="w-full border border-gray-200 rounded-xl px-3 py-2.5 text-sm resize-none" />
                            </div>

                            <button onClick={handleSaleSubmit} disabled={!saleItems.length || saving}
                                className="w-full py-3 bg-blue-600 hover:bg-blue-700 disabled:opacity-50 text-white rounded-xl text-sm font-medium transition-colors">
                                {saving ? (
                                    'Оформление...'
                                ) : (
                                    <span className="flex items-center justify-center gap-1.5">
