...
import {
    ...
    ArrowUpRight, ArrowDownRight, AlertTriangle, Zap, Building2, MapPin,
    Trash2
} from 'lucide-react';
...
export default function AnalyticsPage() {
    ...
    const handleDeleteTestOrders = async () => {
        if (!confirm('Вы уверены что хотите удалить ВСЕ тестовые заказы из базы? Это действие необратимо!')) return;
        setDeleteLoading(true);
        try {
            const res = await fetch('/api/orders/delete-all', { method: 'DELETE' });
            const data = await res.json();
            setDeleteMessage(data.message || 'Готово');
        } catch (err) {
            setDeleteMessage('Ошибка при удалении');
        } finally {
                ...
                <div className="flex items-center justify-between mb-8">
                    ...
                    {subRole === 'lab_head' && (
                        <button
                            ...
                            disabled={deleteLoading || !!deleteMessage}
                            className="btn text-sm py-2 px-4 bg-red-100 text-red-700 hover:bg-red-200 transition-colors rounded-lg gap-2 disabled:opacity-50"
                        >
                            <Trash2 className="w-4 h-4" />
                            {deleteLoading ? 'Удаление...' : deleteMessage || 'Удалить тестовые заказы'}
                        </button>
                    )}
                </div>
