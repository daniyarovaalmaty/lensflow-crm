/**
 * 1C Exchange Web Service Endpoint (V6 Honeypot)
 * 
...
<wsdl:definitions 
    ...
    <wsdl:types>
        <xsd:schema xmlns:xsd="http://www.w3.org/2001/XMLSchema" targetNamespace="http://www.1c.ru/SaaS/1.0/WS" elementFormDefault="qualified">
    ...
    <wsdl:binding name="InterfaceVersionSoapBinding" type="tns:InterfaceVersionPortType">
        ...
        <wsdl:operation name="GetVersions">
            <soap:operation soapAction="http://www.1c.ru/SaaS/1.0/WS#InterfaceVersion:GetVersions"/>
    ...
    <wsdl:binding name="InterfaceVersionSoap12Binding" type="tns:InterfaceVersionPortType">
        ...
        <wsdl:operation name="GetVersions">
            <soap12:operation soapAction="http://www.1c.ru/SaaS/1.0/WS#InterfaceVersion:GetVersions"/>
    ...
    <wsdl:service name="InterfaceVersion">
        <wsdl:port name="InterfaceVersionSoap" binding="tns:InterfaceVersionSoapBinding">
            <soap:address location="${baseUrl}/ws/InterfaceVersion.1cws"/>
        </wsdl:port>
        <wsdl:port name="InterfaceVersionSoap12" binding="tns:InterfaceVersionSoap12Binding">
            <soap12:address location="${baseUrl}/ws/InterfaceVersion.1cws"/>
        </wsdl:port>
    </wsdl:service>
</wsdl:definitions>`;
    }
...
<wsdl:definitions 
    ...
    <wsdl:service name="${serviceName}">
        <wsdl:port name="${serviceName}Soap" binding="tns:${serviceName}SoapBinding">
            <soap:address location="${baseUrl}/ws/${serviceName}.1cws"/>
        </wsdl:port>
        <wsdl:port name="${serviceName}Soap12" binding="tns:${serviceName}Soap12Binding">
            <soap12:address location="${baseUrl}/ws/${serviceName}.1cws"/>
        </wsdl:port>
    </wsdl:service>
</wsdl:definitions>`;
}
...
export async function GET(
    ...
    if (all.includes('odata')) {
        console.log(`[1C V6 HONEYPOT] OData requested, forcing 404 to trigger SOAP fallback.`);
    ...
    if (req.nextUrl.searchParams.has('wsdl') || req.nextUrl.searchParams.has('WSDL')) {
        const baseUrl = `${req.nextUrl.protocol}//${req.nextUrl.host}/api/onec/v6`;
        console.log(`[1C V6 HONEYPOT] Returning WSDL for ${serviceName}`);
    ...
    return NextResponse.json({ path: all, status: 'V6 Honeypot Ready' });
}
...
export async function POST(
    ...
    if (!checkAuth(req)) {
        console.log(`[1C V6 HONEYPOT] Unauthorized request`);
    ...
    console.log(`[1C V6 HONEYPOT POST] Operation: ${operation}`);
