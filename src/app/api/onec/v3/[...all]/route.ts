/**
 * 1C Exchange Web Service Endpoint (V3 Honeypot)
 * 
...
export async function GET(
    ...
    if (all.includes('odata')) {
        console.log(`[1C V3 HONEYPOT] OData requested, forcing 404 to trigger SOAP fallback.`);
    ...
    if (req.nextUrl.searchParams.has('wsdl') || req.nextUrl.searchParams.has('WSDL')) {
        const baseUrl = `${req.nextUrl.protocol}//${req.nextUrl.host}/api/onec/v3`;
        console.log(`[1C V3 HONEYPOT] Returning WSDL for ${serviceName}`);
    ...
    return NextResponse.json({ path: all, status: 'V3 Honeypot Ready' });
}
...
export async function POST(
    ...
    if (!checkAuth(req)) {
        console.log(`[1C V3 HONEYPOT] Unauthorized request`);
    ...
    console.log(`[1C V3 HONEYPOT POST] Operation: ${operation}`);
