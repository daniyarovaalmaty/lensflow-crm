...
export async function GET() {
    ...
    if (org.type === 'headquarters') {
        ...
        if (['optic_doctor', 'optic_ophthalmologist', 'optic_orthokeratologist'].includes(session.user.subRole as string)) {
        ...
        branches = [{ id: org.id, name: org.name, inn: orgFull?.inn, deliveryAddress: orgFull?.deliveryAddress, address: orgFull?.address, directorName: orgFull?.directorName, city: orgFull?.city }] as any;
    }
