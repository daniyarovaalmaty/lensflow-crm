...
export async function GET(req: NextRequest) {
    ...
    for (const org of configured) {
        ...
        const since = lastLog?.syncedAt?.toISOString();

        const t0 = Date.now();
        let results;
        try {
            results = [await svc.syncClientChanges(since)];
        } catch (e: any) {
            results = [{ entity: 'clients', created: 0, updated: 0, errors: 1, details: [e.message] }];
        }
