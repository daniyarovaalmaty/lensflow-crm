/**
 * CreateExchangeNode - put ИмяПланаОбмена inside НастройкиПодключения
 * Error: НастройкиПодключения.ИмяПланаОбмена not found
 * This means the parsed СтрокаПараметровXML fills НастройкиПодключения
 * but ИмяПланаОбмена is not inside it
 * Solution: put ИмяПланаОбмена directly inside НастройкиПодключения Structure
 */
...
async function main() {
  ...
  // Build XML settings string with ИмяПланаОбмена
  const xmlSettings = 
  ...
  console.log('=== CreateExchangeNode with ИмяПланаОбмена in НастройкиПодключения ===\n');

  const formats = [
    {
      name: '1. НастройкиПодключения with ИмяПланаОбмена inside + СтрокаПараметровXML',
        ...
        <tns:Parameters>
          <xs1:Property name="НастройкиПодключения">
            <xs1:Value xsi:type="xs1:Structure">
              ...
              <xs1:Property name="КодУзлаКорреспондента">
                <xs1:Value xsi:type="xsd:string">LENSFLOW</xs1:Value>
              </xs1:Property>
              <xs1:Property name="НаименованиеУзлаКорреспондента">
                <xs1:Value xsi:type="xsd:string">LensFlow CRM</xs1:Value>
              </xs1:Property>
              <xs1:Property name="ПрефиксИнформационнойБазыКорреспондента">
          ...
          <xs1:Property name="СтрокаПараметровXML">
            <xs1:Value xsi:type="xsd:string">${esc(xmlSettings)}</xs1:Value>
          </xs1:Property>
        </tns:Parameters>
        <tns:Zone>0</tns:Zone>
  ...
  for (const fmt of formats) {
    ...
    if (res.body.includes('Fault')) {
      ...
      console.log(`❌ ${fault.substring(0, 1200)}`);
    } else {
