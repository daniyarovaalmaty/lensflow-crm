...
async function migrate() {
    ...
    const configured = orgs.filter(o => {
        const i = typeof o.metadata === 'string' ? JSON.parse(o.metadata)?.itigris : o.metadata?.itigris;
    ...
    for (const org of configured) {
        ...
        for (const letter of ALPHABET) {
            ...
            while (hasMore) {
                ...
                while (retries > 0) {
                    try {
                        res = await fetch(`https://optima.itigris.ru/${cfg.company}/api/v2/clients?clientSearchType=FULL_NAME&searchQuery=${encodeURIComponent(letter)}&deleted=false&page=${page}&size=500`, {
                            headers: { 'Authorization': `Bearer ${token}` }
                        });
                        if (res.ok) break;
                    } catch (e) {
                        console.error(`Fetch failed, retrying... (${retries} left)`, e.message);
                    }
                    retries--;
