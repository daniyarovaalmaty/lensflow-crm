...
async function main() {
    ...
    const searchKeys = ['OCULFIT', 'LLASHP', 'AFR', 'CW', 'VIVOSTAT', 'Gemetrix', 'Ribofast', 'Лампа', 'Ring'];
    for (const [m, q] of Object.entries(totals)) {
        if (searchKeys.some(k => m.toUpperCase().includes(k.toUpperCase()) || k.toUpperCase() === m.toUpperCase())) {
            console.log(`${m}: ${q}`);
        }
    }
}
